...
#         # Declare and get the target_colour parameter
#         self.declare_parameter('target_colour', 'unknown')
#         self.target_colour = self.get_parameter('target_colour').value

#         # Print the received target colour
#         self.get_logger().info(f"TARGET BEACON: Searching for {self.target_colour}.")

#         # Flag to track if we've found the target beacon
#         self.target_beacon_found = False
...
#         camera_topic = '/camera/color/image_raw'  # Default for real robot
#         # camera_topic = '/camera/image_raw' # Uncomment for testing in simulation
...
#         self.safe_distance = 0.5  # Minimum distance to obstacles (metres)
...
#         """
#         Process camera images to detect colored beacons.
#         """
#         try:
...
#             # Only process for beacon detection if we haven't found our target yet
#             if not self.target_beacon_found:
#                 self.detect_target_beacon(cv_image)

#         except CvBridgeError as e:
#             self.get_logger().error(f"CvBridge Error: {e}")

#     def detect_target_beacon(self, image):
#         """
#         Detect the target beacon based on its color.
#         """
#         if self.target_beacon_found:
#             return

#         # Convert to HSV for easier colour detection
#         hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

#         # Define colour ranges for the different beacon colors
#         color_ranges = {
...
#         # Check if target_colour is valid
#         if self.target_colour not in color_ranges:
#             self.get_logger().warn(f"Unknown target color: {self.target_colour}")
#             return

#         # Create mask for the target colour
#         lower_bound, upper_bound = color_ranges[self.target_colour]
...
#             upper_red_mask = cv2.inRange(hsv_image, upper_red_lower, upper_red_upper)
#             mask = cv2.bitwise_or(mask, upper_red_mask)

#         # Find contours in the mask
#         contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

#         # Check if we found any contours of sufficient size
#         if contours:
#             largest_contour = max(contours, key=cv2.contourArea)
#             area = cv2.contourArea(largest_contour)

#             # Only consider contours of a certain size
#             if area > 1000:  # Minimum area threshold
#                 # Get bounding rectangle
#                 x, y, w, h = cv2.boundingRect(largest_contour)

#                 # Check if the beacon is fully in view (near the center and wide enough)
#                 image_width = image.shape[1]
#                 if w > image_width * 0.1 and abs((x + w/2) - image_width/2) < image_width * 0.4:
#                     self.get_logger().info(f"TARGET DETECTED: Beaconing initiated.")

#                     # Save the image
#                     self.save_target_image(image)

#                     # Mark as found
#                     self.target_beacon_found = True

#     def save_target_image(self, image):
#         """
#         Save an image of the target beacon.
#         """
#         snaps_dir = "/home/student/ros2_ws/src/com2009_team53_2025/snaps"

#         # Create directory if it doesn't exist
#         os.makedirs(snaps_dir, exist_ok=True)

#         image_path = os.path.join(snaps_dir, "target_beacon.jpg")

#         # Save the image
#         try:
#             cv2.imwrite(image_path, image)
#             self.get_logger().info(f"Saved target beacon image to: {image_path}")
...
#             maps_dir = "/home/student/ros2_ws/src/com2009_team53_2025/maps"
